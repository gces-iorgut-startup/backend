...
describe('CreateUserUseCase', () => {
  ...
  it('deve criar um usuário com role OWNER', async () => {
    const result = await sut.execute({ name: 'Dr. Gustavo', email: 'gustavo@iougurt.com', password: 'senha123', role: 'OWNER' })
  ...
  it('deve criar um usuário com role VET', async () => {
    const result = await sut.execute({ name: 'Dra. Ana', email: 'ana@iougurt.com', password: 'senha123', role: 'VET' })
    expect(result.role).toBe('VET')
  })

  it('deve fazer hash da senha', async () => {
    await sut.execute({ name: 'Gustavo', email: 'g@g.com', password: 'senha123', role: 'VET' })
  ...
  it('não deve criar usuário com e-mail duplicado', async () => {
    await sut.execute({ name: 'Gustavo', email: 'gustavo@iougurt.com', password: 'a', role: 'VET' })
    await expect(sut.execute({ name: 'Outro', email: 'gustavo@iougurt.com', password: 'b', role: 'VET' })).rejects.toBeInstanceOf(AppError)
  })

  it('deve lançar AppError com statusCode 409 para e-mail duplicado', async () => {
    await sut.execute({ name: 'Gustavo', email: 'g@g.com', password: 'a', role: 'VET' })
    await expect(sut.execute({ name: 'Outro', email: 'g@g.com', password: 'b', role: 'VET' })).rejects.toMatchObject({ statusCode: 409 })
  })
})
